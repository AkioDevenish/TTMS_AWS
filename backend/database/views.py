...
from .serializers import (
    ...
    StationSerializer,
    StationHealthLogSerializer,
    StationSensorSerializer,
    ApiAccessKeySerializer,
    SystemLogSerializer,
    UserSerializer,
...
class StationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Station.objects.select_related("brand").prefetch_related(
            "station_sensors__sensor", "health_logs"
        )
        ...
        include_decommissioned = (
            self.request.query_params.get("include_decommissioned", "false").lower()
            == "true"
        )
...
class MeasurementViewSet(viewsets.ModelViewSet):
    ...
    def highest_by_brand(self, request):
        ...
        try:
            ...
            for station in stations:
                ...
                sensor_ids = station_sensors.values_list(
                    "sensor_id", flat=True
                ).distinct()
                ...
                for sensor in sensors:
                    sensor_measurements = station_measurements.filter(
                        sensor_id=sensor.id
                    )
                ...
                for sensor in sensors.filter(id__in=sensor_ids):
                    ...
                    latest = (
                        Measurement.objects.filter(
                            station_id=station.id, sensor_id=sensor.id
                        )
                        .order_by("-date", "-time")
                        .first()
                    )
        ...
        try:
            ...
            for station in stations:
                ...
                measurements_query = Measurement.objects.filter(
                    station_id=station.id, date_time__gte=time_threshold
                ).order_by("-date_time")

                # Apply sensor type filter if provided
                if sensor_type:
                    measurements_query = measurements_query.filter(
                    ...
                    result.append(
                        {
                            ...
                            "time": m.time,
                            "date_time": (
                                m.date_time.isoformat()
                                if m.date_time
                                else f"{m.date}T{m.time}"
                            ),
                            "value": m.value,
    ...
    def measurements_by_brand(self, request):
        ...
        try:
            ...
            print(
                f"Getting time series data for brand: {brand}, sensor: {sensor_type}, hours: {hours}"
            )
            ...
            for station in stations:
                ...
                print(
                    f"Found {len(measurements)} measurements for station {station.name}"
                )
    ...
    def station_overview(self, request):
        try:
            ...
            # Create cache key
            cache_key = (
                f"station_overview_{brand}_{sensor_type}_{page}_{page_size}_{latest}"
            )
            cached_data = cache.get(cache_key)
            ...
            if sensor_type:
                ...
                print(
                    f"Filtering stations by sensor type: {sensor_type} -> {mapped_sensor_type}"
                )
                ...
                print(
                    f"Found {stations_with_sensor.count()} stations with sensor type {mapped_sensor_type}"
                )
                ...
                if stations_with_sensor.exists():
                    ...
                    print(
                        f"Applied sensor filtering, showing {stations.count()} stations"
                    )
                else:
                    print(
                        f"No stations found with sensor type {mapped_sensor_type}, showing all stations"
                    )
            ...
            for station in stations:
                ...
                if sensor_type:
                    ...
                    print(
                        f"Station {station.name}: Looking for measurements with sensor type {mapped_sensor_type} from {yesterday.date()}"
                    )
                    print(f"  Query found {measurement_query.count()} measurements")

                    latest_measurement = measurement_query.order_by(
                        "-date", "-time"
                    ).first()

                    if latest_measurement:
                        print(
                            f"  Latest measurement: {latest_measurement.date} {latest_measurement.time} - Value: {latest_measurement.value}"
                        )
                    else:
                ...
                station_data = {
                    ...
                    "latest_measurement": (
                        {
                            "value": (
                                float(latest_measurement.value)
                                if latest_measurement
                                else None
                            ),
                            "date": (
                                latest_measurement.date.isoformat()
                                if latest_measurement
                                else None
                            ),
                            "time": (
                                self.convert_time_to_local(
                                    latest_measurement.date, latest_measurement.time
                                )
                                if latest_measurement
                                else None
                            ),
                            "status": (
                                latest_measurement.status
                                if latest_measurement
                                else "No Data"
                            ),
                        }
                ...
                if not latest_measurement:
                    any_recent_measurement = (
                        Measurement.objects.filter(
                            station_id=station.id, date__gte=yesterday.date()
                        )
                        .order_by("-date", "-time")
                        .first()
                    )
                    ...
                    if any_recent_measurement:
                        print(
                            f"  Station {station.name}: No {sensor_type} data, but has recent {any_recent_measurement.sensor.type} data"
                        )
                        station_data["station_health"]["has_any_recent_data"] = True
                        station_data["station_health"][
                            "last_data_time"
                        ] = any_recent_measurement.date.isoformat()
                    else:
    ...
    def history(self, request):
        ...
        try:
            ...
            print(
                f"Parsed parameters: station_ids={station_ids_param}, sensor_type={sensor_type_param}, hours={hours}"
            )
            ...
            sensor_types = [
                s.strip() for s in sensor_type_param.split(",") if s.strip()
            ]
            if not sensor_types:
            ...
            stations = Station.objects.filter(id__in=station_ids_list).select_related(
                "brand"
            )
            ...
            measurements = (
                Measurement.objects.filter(
                    station_id__in=station_ids, sensor__type__in=sensor_types
                )
                .filter(
            ...
            result = {"measurements": result_data}
            ...
            if (
                ...
                try:
                    ...
                    print(
                        f"Attempting to create ApiKeyUsageLog with data: {log_data}"
                    )  # Debug print log data
                    ...
                    print(
                        f"SPECIFIC Error logging API key usage: {type(e).__name__} - {str(e)}"
    ...
    def get_readings(self, request, format=None):
        ...
        try:
            ...
            start_date = request.query_params.get(
                "start_date", default_start.date().isoformat()
            )
...
class StationHealthLogViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        latest_logs = (
            StationHealthLog.objects.values("station")
        ...
        queryset = (
            StationHealthLog.objects.filter(id__in=latest_logs)
...
class ApiAccessKeyViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        print(f"user_id value: {user_id}")  # Print user_id to the console
        if user_id:
            return ApiAccessKey.objects.filter(user_id=user_id)
...
class SystemLogViewSet(viewsets.ModelViewSet):
    ...
    def by_module(self, request):
        ...
        if not module:
            return Response(
                {"error": "module is required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # If the key is perpetual, ensure expires_at is None
        if is_perpetual is True:
            data["expires_at"] = None

        # Validate expires_at format
        if not is_perpetual and "expires_at" in data and data["expires_at"]:
            try:
        ...
        if serializer.is_valid():
            ...
            if user.status == "Active":
                try:
                    ...
                    logger.error(
                        f"Failed to create API key for new user {user.email}: {str(e)}"
                    )
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if original_status != "Active" and updated_user.status == "Active":
            try:
                ...
                logger.info(
                    f"API key {api_key.uuid} created for activated user {updated_user.email}"
                )
                ...
                logger.error(
                    f"Failed to create API key for activated user {updated_user.email}: {str(e)}"
                )
    ...
    def generate_api_key(self, user, is_perpetual):
        ...
        try:
            ...
            if not email_sent:
                logger.warning(
                    f"API key created for {user.email} but email notification failed"
                )
...
class UserList(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if self.request.user.email == "mdpssupport@metoffice.gov.tt":
            return User.objects.exclude(email="mdpssupport@metoffice.gov.tt").order_by(
                "-created_at"
            )
...
class ChatListCreate(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        return (
            Chat.objects.filter(
                Q(user=user) | Q(participants=user) | Q(support_chat=True)
            )
            .select_related("user")
            .prefetch_related("messages", "participants")
            .order_by("-created_at")
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            message = serializer.save(
                chat=chat, sender=request.user, time=timezone.now().time()
            )
...
class BillViewSet(viewsets.ModelViewSet):
    ...
    def receipt_upload(self, request, pk=None):
        ...
        response["Content-Disposition"] = (
            f'inline; filename="{bill.receipt_upload.name}"'
        )
        return response
...
def get_task_execution_status(request):
    try:
        ...
        for task in tasks:
            ...
            time_until_next = max(
                0, task.interval - (time_since_last_run % task.interval)
            )

            # Calculate progress percentage
            progress = min(
                100, (time_since_last_run % task.interval) / task.interval * 100
            )
...
def get_stations_status(request):
    try:
        ...
        stations = Station.objects.exclude(status="Decommissioned").select_related(
            "brand"
        )
        current_time = timezone.now()
        ...
        for station in stations:
            ...
            if latest_health:
                time_since_last = (
                    current_time - latest_health.created_at
                ).total_seconds()
            ...
            station_statuses.append(
                {
                    ...
                    "brand": station.brand.name,
                    "status": (
                        latest_health.connectivity_status
                        if latest_health
                        else "Unknown"
                    ),
...
def get_stations_comprehensive_status(request):
    ...
    try:
        ...
        for station in stations:
            ...
            latest_measurements = all_measurements[
                :100
            ]  # Last 100 measurements for display
            ...
            if not has_recent_transmission:
                ...
                status_code = "offline"
                status_description = (
                    "Station not reporting data (no recent transmissions)"
                )
                ...
                status_code = "online"
                status_description = (
                    "Station reporting valid data within expected parameters"
                )
            ...
            if latest_measurements.exists():
                time_since_last = (
                    current_time - latest_measurements.first().created_at
                ).total_seconds()
            ...
            station_statuses.append(
                {
                    ...
                    "transmission_status": (
                        "Active" if has_recent_transmission else "Inactive"
                    ),
                    ...
                    "data_quality_metrics": {
                        ...
                        "invalid_percentage": (
                            round((invalid_data_count / total_data_count) * 100, 2)
                            if total_data_count > 0
                            else 0
                        ),
                    },
                    "last_updated": (
                        latest_measurements.first().created_at
                        if latest_measurements.exists()
                        else None
                    ),
                    ...
                    "health_info": {
                        "battery_status": (
                            latest_health.battery_status if latest_health else "Unknown"
                        ),
                        "connectivity_status": (
                            latest_health.connectivity_status
                            if latest_health
                        ...
                        "last_health_check": (
                            latest_health.created_at if latest_health else None
                        ),
                    },
                }
            )
        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "online": len(
                        [s for s in station_statuses if s["status_code"] == "online"]
                    ),
                    "offline": len(
                        [s for s in station_statuses if s["status_code"] == "offline"]
                    ),
                    "online_erroneous": len(
                        [
                            s
                            for s in station_statuses
                            if s["status_code"] == "online_erroneous"
                        ]
                    ),
                },
        ...
        .annotate(
            latest_battery_status=Subquery(
                StationHealthLog.objects.filter(
                    station=OuterRef("pk"), created_at__gte=one_hour_ago
                )
                .order_by("-created_at")
...
def station_health_logs(request):
    ...
    try:
        ...
        # Get parameters
        brands = request.query_params.get(
            "brands", "3D_Paws,Allmeteo,Zentra,OTT,Sutron,AWS"
        )
        brand_list = brands.split(",")

        # Annotate each station with the latest health log id
        latest_log_subquery = (
            StationHealthLog.objects.filter(station=OuterRef("pk"))
            .order_by("-created_at")
            .values("id")[:1]
        )

        stations = (
            Station.objects.filter(brand__name__in=brand_list)
            .annotate(latest_log_id=Subquery(latest_log_subquery))
            .select_related("brand")
        )
        ...
        print(traceback.format_exc())
        return Response(
            {"data": [], "total": 0, "page": 1, "page_size": 100, "error": str(e)}
        )
...
def station_temperature_overview(request):
    ...
    for station in stations:
        ...
        latest_temp = (
            station.measurements.filter(
                parameter_code__in=["bt1", "mt1"], date_time__gte=yesterday
            )
            .order_by("-date_time")
            .first()
        )
        ...
        response_data.append(
            {
                ...
                "battery_status": (
                    station.latest_health.battery_status
                    if station.latest_health
                    else "Unknown"
                ),
    ...
    try:
        ...
        for station in stations_queryset:
            ...
            if log:
                battery_status = log.battery_status
                if (
                    log.connectivity_status
                    and log.connectivity_status != "Unknown"
                    and log.connectivity_status != "No Data"
                ):
                    connectivity_status = log.connectivity_status
                    ...
                    if battery_status != "Unknown" and battery_status != "No Data":
                        ...
                        if (
                            log.created_at
                            and (timezone.now() - log.created_at).total_seconds()
                            < 86400
                        ):  # 24 hours
                            status = "Online"
                        else:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Updated {aws_stations.count()} AWS stations")
        )
...
def inactive_sensors(request):
    try:
        ...
        )  # Default to 20 as per frontend component
        ...
        recent_sensors = (
            Measurement.objects.filter(
                station_id__in=station_ids, date__gte=seven_days_ago.date()
            )
            .values_list("station_id", "sensor_id")
            .distinct()
        )
        ...
        all_configured_sensors = (
            Measurement.objects.filter(station_id__in=station_ids)
            .values_list("station_id", "sensor_id")
            .distinct()
        )
        ...
        stations_with_sensors = stations_queryset.prefetch_related(
        ...
        for station in stations_with_sensors:
            ...
            for station_sensor in station_sensors:
                ...
                if not has_recent_data:
                    ...
                    if latest:
                        last_reading_dt = (
                            ...
                            if latest.date and latest.time
                            else None
                        )
                ...
                if latest:
                    last_reading_dt = (
                        timezone.make_aware(datetime.combine(latest.date, latest.time))
                        if latest.date and latest.time
                        else None
                    )
                    ...
                    all_inactive_sensors_list.append(
                        {
                            ...
                            "last_reading": (
                                last_reading_dt.isoformat() if last_reading_dt else None
                            ),
                            "status": status,
                        }
                    )
        ...
        for i, sensor in enumerate(paginated_inactive_sensors[:5]):
            print(
                f"  {i+1}. {sensor['station_name']} -> {sensor['sensor_type']} -> Status: '{sensor['status']}' -> Last: {sensor['last_reading']}"
            )

        # Add debugging information
        print(f"DEBUG: Total inactive sensors found: {total_count}")
        print(
            f"DEBUG: Sensors with recent measurements (7 days): {len(recent_sensors_set)}"
        )
        ...
        print(
            f"DEBUG: Performance improvement: Skipped {len(stations_with_sensors) * len(stations_with_sensors[0].station_sensors.all()) - len(configured_sensors_set)} unconfigured sensor combinations"
        )
        ...
        if brand == "OTT" or not brand:
            ...
            for station in ott_stations:
                station_sensors = station.station_sensors.all()
                print(
                    f"DEBUG: OTT Station {station.name} has {len(station_sensors)} sensor relationships"
                )
                for ss in station_sensors[:3]:  # Show first 3 sensors
                    sensor = ss.sensor
                    if sensor:
                        latest_measurement = (
                            Measurement.objects.filter(station=station, sensor=sensor)
                            .order_by("-date", "-time")
                            .first()
                        )
                        if latest_measurement:
                            print(
                                f"DEBUG:   Sensor {sensor.type}: Latest measurement {latest_measurement.date} {latest_measurement.time}"
                            )
                        else:
                            print(
                                f"DEBUG:   Sensor {sensor.type}: No measurements found"
                            )
        ...
        if all_inactive_sensors_list:
            ...
            for i, sensor in enumerate(all_inactive_sensors_list[:10]):
                print(
                    f"  {i+1}. {sensor['station_name']} -> {sensor['brand_name']} -> {sensor['sensor_type']} -> Status: '{sensor['status']}' -> Last Reading: {sensor['last_reading']}"
                )
        else:
        ...
        t10_sensors = [
            s for s in all_inactive_sensors_list if "T10" in s["station_name"]
        ]
        if t10_sensors:
            print(f"DEBUG: T10 Paramin sensors found: {len(t10_sensors)}")
            for sensor in t10_sensors:
                print(
                    f"  - {sensor['station_name']} -> {sensor['sensor_type']} -> {sensor['status']}"
                )
        else:
    ...
    def get_queryset(self):
        ...
        if api_key_uuid:
            try:
                ...
                )  # Filter by the ApiAccessKey object
