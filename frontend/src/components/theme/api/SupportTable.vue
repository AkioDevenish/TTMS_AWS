<template>
  <div v-if="isAdmin" class="table-responsive">
      ...
      <tbody v-else>
        <tr v-for="(row, index) in get_rows()" :key="row.id" class="api-key-row">
          ...
          <td class="status-cell px-4 py-3">
            <span :class="[ 'status-badge', `status-${getStatus(row).toLowerCase()}`]">
              {{ getStatus(row) }}
            </span>
          </td>
          <td class="px-4 py-3">
    ...
    <div class="mt-3 text-center" v-if="num_pages() > 1">
      ...
      <button class="btn btn-outline-primary ms-2" @click="change()" :disabled="currentPage === num_pages()">Next
      </button>
    </div>
  </div>
...
function getStatus(row: any) {
  if (row.is_perpetual === true) return 'Active';
  if (row.expires_at) {
