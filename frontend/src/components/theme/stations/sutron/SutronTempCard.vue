<template>
    <div>
      <div class="row g-2">
        <div class="col-xl-6 col-lg-12 box-col-12 proorder-md-3" v-for="(item, index) in localSutronData" :key="index">
  ...
  const sensorConfig: Record<string, { name: string; unit: string; threshold: number }> = {
    ...
    'humidity': { name: 'Relative Humidity', unit: '%', threshold: 1 },
    'pressure': { name: 'Atmospheric Pressure', unit: 'hPa', threshold: 0.5 },
    'wind_speed': { name: 'Wind Speed', unit: 'm/s', threshold: 0.2 },
    'wind_direction': { name: 'Wind Direction', unit: '°', threshold: 5 },
    'rainfall': { name: 'Rainfall', unit: 'mm', threshold: 0.1 },
    'solar_radiation': { name: 'Solar Radiation', unit: 'W/m²', threshold: 1 },
    'battery': { name: 'Battery', unit: 'V', threshold: 0.1 },
    'dew_point': { name: 'Dew Point', unit: '°C', threshold: 0.1 },
    'barometric_pressure': { name: 'Barometric Pressure', unit: 'hPa', threshold: 0.5 },
    'wind_gust': { name: 'Wind Gust', unit: 'm/s', threshold: 0.2 },
    'wind_gust_direction': { name: 'Wind Gust Direction', unit: '°', threshold: 5 },
    'solar_radiation_avg': { name: 'Solar Radiation Average', unit: 'W/m²', threshold: 1 },
    'solar_radiation_total': { name: 'Solar Radiation Total', unit: 'W/m²', threshold: 1 }
  };
  ...
  watch(() => measurements.value, (newMeasurements) => {
    if (!newMeasurements?.length) {
      localSutronData.value = [];
      return;
    }
    localSutronData.value = transformMeasurements(newMeasurements);
  }, { immediate: true });
  </script>
