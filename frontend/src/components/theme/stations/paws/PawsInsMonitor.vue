                        ...
                        <tr v-for="(item, index) in latestData" :key="index">
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="d-flex align-items-center">  
                                        <h6>{{ item.name }}</h6>
                                        <i class="status-dot" :class="{ 'online': item.status === 'Online', 'offline': item.status === 'Offline' }"></i>
                                    </div>
                                </div>
                            </td>
                            <td>{{ item.id }}</td>
...
watch([
    ...
    try {
        ...
        if (recentMeasurement) {
            const m: any = recentMeasurement;
            status = 'Online';
            lastUpdated = `${m.date}T${m.time}`;
            const measurementTime = new Date(`${m.date}T${m.time}`).getTime();
            ...
            const measurementTime = new Date(`${m.date}T${m.time}`).getTime();
            const diffMinutes = Math.round((now - measurementTime) / (1000 * 60));
            lastUpdatedAgo = `${diffMinutes} min ago`;
        }
        latestData.value = [{
