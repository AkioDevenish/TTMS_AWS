  ...
  <div class="col-md-12">
    <div class="card">
      ...
      <div v-else-if="apiKeys.length === 0" class="p-4 text-center">
        <p>No API keys available for this user.</p>
      </div>
...
const fetchApiKeys = async () => {
  ...
  try {
    ...
    // Get profile ID from route or from current logged-in user
    let profileId = route.query.id || currentUser.value?.id;
    console.log('Profile ID from route:', profileId);

    if (!profileId) {
      console.warn('No profile ID found to query API keys.');
...
const isExpired = (key: any) => {
  if (key.is_perpetual) return false;
